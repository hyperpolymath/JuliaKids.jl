...
All placeholders in this template follow the `JuliaKids.jl` pattern.
...
sed -i 's/JuliaKids.Jl/my-project/g' $(grep -rl 'JuliaKids.Jl' .)
sed -i 's/JuliaKids.jl/MY_PROJECT/g' $(grep -rl 'JuliaKids.jl' .)
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `JuliaKids.Jl` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.guix, devcontainer.json |
| `JuliaKids.jl` | One-line description | `A tool for X` | flake.guix |
| `JuliaKids.jl` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/abi/*.idr, ffi/zig/*.zig |
| `{{project}}` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, ffi/zig/*.zig |
| `JuliaKids.jl` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
...
| `JuliaKids.jl` | One-line project description | `FFI bridges between languages` | STATE.a2ml |
